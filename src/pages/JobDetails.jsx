        ...
        filteredJobs.slice(0, visible).map((item, index) => (
          <li key={index}>
            ...
            <div className="job-card-right">
              ...
              <p className="type">{item.jobType} | {item.timePeriod}</p>
              <p className="description">{item.description}</p>
            </div>
            <button className="apply" onClick={handleapply}> Apply job</button>
            <button className="Save-btn" onClick={() => saveJob(item)}>☆</button>
          </li>
        ))
      )}
    </ul>
